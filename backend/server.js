...
app.get('/slist', async(req, res) => {
    ...
    const objects = await client.db('Tiktiok').collection('DataTiktok').find({}).toArray();
    await client.close();
...
app.post('/slist/create', async(req, res) => {
    ...
    const client = new MongoClient(uri);
    await client.connect();
    await client.db('Tiktok').collection('DataTiktok').insertOne({
...
app.put('/slist/update', async(req, res) => {
    ...
    const client = new MongoClient(uri);
    await client.connect();
    await client.db('Tiktiok').collection('DataTiktok').updateOne({'_id': ObjectId(id)}, 
    {"$set": {
        "StudentID": object['StudentID'],
        "Title": object['Title'],
        "Name": object['Name'],
...
app.delete('/slist/delete', async(req, res) => {
    ...
    const client = new MongoClient(uri);
    await client.connect();
    await client.db('Tiktiok').collection('DataTiktok').deleteOne({"_id": ObjectId(id)});
    await client.close();
    res.status(200).send({
        "status": "ok",
...
app.get('/slist/field/:searchText', async(req, res) => {
        ...
        const client = new MongoClient(uri);
        await client.connect();
        const objects = await client.db('Tiktiok').collection('DataTiktok').find({ $text: {$search: searchText } }).sort({ "Date received": -1 }).limit(5).toArray();
        await client.close();
        res.status(200).send({
          "status": "ok",
...
app.get('/slist/:id', async(req, res) => {
        ...
        const client = new MongoClient(uri);
        await client.connect();
        const object = await client.db('Tiktiok').collection('DataTiktok').findOne({ "_id": ObjectId(id) });
        await client.close();
        res.status(200).send({
            "status": "ok",
            "ID": id,
